# todo

## AI 시대 학습 프레임워크 사이트 반영 (2026-08-11)

### 콘텐츠 추가
- [x] `learning-design/` 디렉터리 생성, 원본 3종 배치
  - [x] `index.html` ← learning-design.html (본편 · 지형·경로·장비)
  - [x] `triangulation.html` ← 부록 A · 삼각측량
  - [x] `yajang.html` ← 부록 B · 야장
- [x] 3종 문서 상단에 사이트 바 삽입 (연습장 복귀 + 문서 간 상호 이동)
- [x] `ai-learning.html` 생성 — 사이트 톤(notebook.css)의 소개 페이지
  - [x] 세 단계(지형·경로·장비) 요약
  - [x] Gate Rule + 네 박자 AI 배치
  - [x] 질문고리 학습과의 비교표
  - [x] 원문 3종으로 연결
- [x] `philosophy.html` 여는 글 — “교육은 전달이 아니라 스스로 발생하는 것이다”
      × 핀켈 『침묵으로 가르치기』 × AI 시대의 침묵

### 사이트 개선
- [x] 전 페이지 topnav 통일 (14개 페이지 · 7개 항목)
  - `academy.html`에 빠져 있던 학습설계 복구
  - `solo-builders/` 의 철학 → 프레임워크, brand-mark 표기 통일
- [x] `sitemap.xml` 갱신 — 누락분(philosophy, monetization-worksheet) + 신규 4개
- [x] 전 페이지 og:url / og:image / twitter:card 보완 (14개 페이지)
- [x] `index.html` — AI학습 카드 추가, 여는 글로 가는 링크 추가
- [x] `framework.html` ↔ `ai-learning.html` ↔ `solo-learning.html` 상호 링크
- [x] `README.md` 표 · 디렉터리 트리 갱신

### 확인
- [x] 로컬 링크 192개 전수 검사 — 깨진 링크 없음
- [x] sitemap 17개 URL 전부 실제 파일과 일치
- [x] 로컬 서버 주요 페이지 200 응답

---

## learning-design 3종 디자인 결 맞추기 (2026-08-11)

- [x] 폰트 교체 — Gothic A1 / IBM Plex → Noto Sans KR / JetBrains Mono
- [x] 팔레트 리맵 — riso 필드매뉴얼(청록·형광주황) → 연습장 모노크롬
  - paper #E8ECE3 → #FFFFFF · ink #14312B → #0A0A0A · rule → #EAEAEA
  - fluoro #FF5B26 → #0A0A0A · teal #1E6055 → #6E6E6E · blue #2A4BA0 → #6E6E6E
- [x] 측량 그리드 배경 제거 (사이트의 흰 종이에 맞춤)
- [x] 리소 오버프린트(색 어긋남) 효과 해제 — 모노크롬에서는 번짐으로 보임
- [x] `mark` 형광 마커 → 연습장 반전 마커(검정 배경 + 흰 글자)
- [x] 금지 목록 박스 → 사이트 `.callout` 패턴(3px 좌측 먹선)
- [x] SVG 다이어그램 색·폰트 178건 치환 (등고선은 흐린 회색으로 후퇴)
- [x] 사이트 크롬 — sticky topbar(브랜드 + 7개 nav, AI학습 active) + 문서 바 + 푸터
- [x] 링크 215개 전수 재검사 · 팔레트 토큰 notebook.css와 일치 확인

---

## 부록 C · 첫 말뚝 — 문제 정의와 질문 생성 (2026-08-11)

두 프레임워크가 "진짜 질문에서 시작한다"고 선언한 뒤 건너뛴 자리를 메움.
Station 00이 없던 문제.

- [x] `learning-design/malttuk.html` 신규 (부록 C, 학습자용 · 진행자 트랙 없음)
  - [x] C0 전제 — 두 프레임워크의 빈칸 4곳 적시
  - [x] C1 문제 — 문제 진술 4줄(입력형) · is/is-not · 리프레이밍 5동작 · 5 Whys 한계 · 분해 테스트
  - [x] C2 질문 — 질문 폭발(4분 타이머) · 한 바퀴(20분) · 판정 4기준 · 검문소(인터랙티브) · 줄기 8종 · 동료 검증 5문
  - [x] C3 AI — Gate Rule C · 위임 경계 6행 · 금지 목록 · 프롬프트 3종
  - [x] C4 유효성 — 근거 8건 + 정직한 한계
  - [x] C5 실행 — 60분 첫 바퀴 + 내려받기/인쇄/비우기
- [x] 판정 4기준을 **사이트 기존 어휘로만** 조립 (신규 개념 0개)
      판단형 / 깨지는 가설 / 루프 크기 / 불확실성 축소
- [x] 문서 4종 docbar에 부록 C 추가
- [x] `framework.html` ①문제 설정 · 동료 검증 콜아웃에서 링크
- [x] `ai-learning.html` Station 01 콜아웃 + 문서 카드(3→4, 2x2 그리드)
- [x] `library.html` 「문제 세우기 & 질문 만들기」 선반 신설 (6권)
- [x] sitemap 추가 · 링크 237개 전수 검사 · CSS 클래스 정의 확인

---

## 메인을 페인 포인트 라우터로 전환 (2026-08-12)

gongysd.com 구조 참고. 「연습장의 칸들」 7장 카드 = 만든 것을 나열한 장식장이라
방문자가 자기 문제를 못 찾던 문제. 호명 → 자기 확인 → 경로 분기로 재구성.

- [x] `index.html` 재작성
  - [x] 히어로에 자기 호명 3줄 (응원 메시지 원문은 유지)
        ~~롤링 애니메이션~~ → 정적 나열로 변경. 읽는 데 방해만 됨.
  - [x] 「지금 어디에서 막혀 있나요?」 라우터 신설 — 배우다 / 가르치다 / 만들다 3트랙
  - [x] 트랙당 증상 5개 × 3 = 15개, 각 증상이 1인칭 문장 + 진단 + 도착지
  - [x] 각 패널 하단에 "내 문장이 없다면 → 편지함" 탈출구
  - [x] 「연습장의 칸들」 카드 삭제 → 하단 「전체 목차」 4열 그룹 링크로 강등
  - [x] 두 가지 풍경 → 「Value · 여기 있는 건 전부 설명이 아니라 설계입니다」로 개편
        위치는 히어로 다음 · 라우터 앞. gongysd의 Hero → Values → System 순서와 동일.
        (먼저 무엇을 주는 곳인지 밝히고, 그다음에 고르게 함)
  - [x] SEEDS → 「모든 설계도가 도는 다섯 단계」로 연결 문구 수정
  - [x] meta description·og:description을 페인 포인트 문장으로 교체
- [x] 접근성 — role=tablist/tab/tabpanel, aria-selected, ← → 키 이동, JS 없으면 3패널 모두 노출
- [x] `#learn` / `#teach` / `#build` 해시 딥링크 + hashchange 대응
- [x] 검증 — 내부 링크 21개 전수 OK · 태그 균형 OK · JS 문법 OK · 미정의/미사용 CSS 0

---

## 세상 → 밸류 → 도구 체인 (2026-08-12)

"내가 추구하는 세상이 있고, 밸류가 있고, 그게 실천되는 게 도구다"를
메인에 그대로 흐르게 함. 도구 27개는 **만든 사람 기준**(배우다/실험하다/
만들다/나누다) → **쓰는 사람이 덜게 되는 것 기준**으로 재분류.

- [x] `index.html`
  - [x] World — 「제가 만들고 싶은 세상은 이렇게 생겼습니다」 (paradigm·프레이리 흡수)
  - [x] Value — 3장 카드: 스스로 하게 / 손에 잡히게 / 누구에게나 열리게
  - [x] Practice — 「그래서 만든 것들」 도구 카테고리 7장 (대표 앱 + 개수 + tools 앵커)
  - [x] 라우터는 유지, Practice 아래로 이동
- [x] `tools.html` 27개 전수 재편성 + 카테고리 앵커 + 칩 네비
  - reduce 4 · plan 4 · see 4 · catch 4 · start 5 · check 3 · play 3 = 27
  - [x] 설명문을 기능 나열 → "무엇을 덜게 되는지"로 다듬음
        (Rereminder "끝나기 전에 미리", DontGoMart "헛걸음하지 않게" 등)
- [x] 검증 — URL·도구명 27개 git HEAD와 완전 일치(유실 0) · 앵커 양방향 정합 ·
      태그 균형 · JS 문법 · 미정의/미사용 CSS 0

---

## 대상 좁히기 — 세 칸 사다리 (2026-08-12)

"도움을 주고 싶은 사람부터 명확하게." 대상을 셋으로 좁힘.
가장 큰 변화: **가르치는 사람을 1차 대상에서 뺌.**

- [x] `index.html` 최상단에 Who 섹션 신설 (Hero 바로 다음)
  - 01 배우는 법을 배우고 싶은 사람 — 다음 배움이 빨라지는 구조 = 복리
  - 02 그 과정에 필요한 도구를 얻고 싶은 사람 — 손이 덜 가야 머리가 남음
  - 03 그 도구를 직접 만들어 쓰고 싶은 사람 — 복리의 마지막 칸
  - [x] 셋은 병렬이 아니라 **포함 관계 사다리**로 표현 (.ladder / .rung)
  - [x] 각 칸에 1인칭 인용문 배치 (자기 확인용)
  - [x] 「범위 밖」 콜아웃 — 가르치는 분용 자료는 있지만 1차 대상 아님을 명시
- [x] 라우터 트랙 교체: 배우다 / ~~가르치다~~ / 만들다 → **배우다 / 갖추다 / 만들다**
  - [x] `panel-teach` → `panel-equip` (도구 카테고리 5개로 연결)
  - [x] 배우다 05를 아카데미 → framework(복리가 안 붙는다)로 교체
  - [x] 만들다 05를 도구서랍 → tools#start(만들 줄 모른다)로 교체해 사다리 연결
  - [x] 해시 딥링크 #learn / #equip / #build
- [x] 히어로 롤링 티커 제거 → Who 섹션이 그 역할을 대신함 (CSS·JS 잔재 0)
- [x] 전체 목차를 01/02/03 + 곁에 두는 것 + 「가르치는 자리에 계신 분께」로 재편
- [x] meta·og description을 세 칸 사다리 문구로 교체
- [x] 검증 — 탭↔패널 키 일치 · 증상 5/5/5 · 내부 앵커 0 파손 ·
      index↔tools 앵커 양방향 정합 · 태그 균형 · JS 문법 · CSS 정합

---

## 성장 기술 열둘 — 색인 페이지 (2026-08-15)

프레임워크가 넷으로 늘었는데 "무엇을 기르는 것인지"를 말하는 자리가 없었음.
HypoLoop(가설순환학습)을 엔진으로 두고, 그 **위에** 능력 축 색인을 얹음.
프레임워크 = 시간 축(순서·동사) / 이 페이지 = 능력 축(목록·명사).

- [x] `skills.html` 신규 — 3층 × 12기술
  - [x] 1층 돌리는 기술 5개 — HypoLoop 8단계에 1:1 매핑 (Step 배지 표기)
  - [x] 2층 계속 돌리는 기술 4개 — 습관 / 에너지·시간 / 주의 / 다시 시작하기
  - [x] 3층 크게 돌리는 기술 3개 — 기록 / 도구로 만들기 / 내놓기
  - [x] 12칸 미터로 채움 8 · 빈칸 4를 첫 화면에서 보이게
  - [x] 빈칸 4개는 전부 2층 — 사선 해칭 + 「빈칸」 배지로 명시
  - [x] 「이 목록을 늘리는 규칙」 4조 (엔진 하나 · 프레임워크 층당 하나 ·
        부록 우선 · 템플릿은 싸다) — 프레임워크 증식 방지
- [x] 검증 — 내부 링크 12개 전수 OK · 앵커 정합 · 태그 균형 · 미정의 CSS 0

- [x] `frameworks.html` 허브에 fw-index 입구 · framework.html 하단 링크 · sitemap 등재
- [x] 이름은 **QuestLoop / 질문고리 학습**으로 확정 (HypoLoop 아님)

### 이어서 결정할 것
- [ ] 2층 프레임워크 설계 — 새로 만들 정당성이 있는 유일한 자리
- [ ] 2층 4칸용 템플릿 (종이 한 장짜리) — 프레임워크보다 먼저 붙일 수 있음
- [ ] 1층 5칸 도착지가 겹침 (02·04·05가 전부 framework.html) — 앵커 필요
- [ ] skills.html topnav 노출 여부 — 현재는 frameworks/framework 경유로만 접근

---

## 연구 노트 자리 신설 + 고아 페이지 복구 (2026-08-15)

`thoughts.html`(생각)이 **유입 링크 0개**로 완전한 고아였음 — nav를 4개로 줄일 때
잘려나감. 글 11편 + 발표 6건이 통째로 접근 불가 상태였던 것.
동시에 사이트 전체에 "결론이 안 난 글"을 담을 자리가 없었음.
질문고리 학습은 "깨질 가설을 먼저 써라"인데 정작 사이트엔 깨질 글이 0편.

- [x] topnav에 「생각」 추가 — **18개 페이지 전수**
      순서: ABOUT · 생각 · 성장 프레임워크 · 도구 · 도구 만들기
      (사다리 3칸을 연속으로 두고, 생각은 ABOUT 옆 '사람' 영역에 배치)
- [x] `data/notes.json` 신설 — **카테고리를 데이터로 분리**
      categories[] 에 { id, name, icon, desc } 추가하면 페이지 수정 없이 반영
      초기 2종: 성장에 대한 것 🌱 / 일에 대한 것 🛠
- [x] `thoughts.html` 「연구 노트」 섹션 — notes.json 읽어 카테고리별 렌더
      - 노트 0건이면 섹션 자체를 숨김 (빈 껍데기 노출 방지)
      - JSON 미등록 카테고리는 뒤에 자동 편입
      - status: thinking(생각 중·점선 배지) / settled(정리됨·실선)
- [x] `notes/_template.html` — 노트 본문 템플릿
      「지금 생각은 여기까지」 블랙 블록 + 「고친 자리」 changelog
- [x] 검증 — 렌더 로직 node 실행(3건/3카테고리/미등록 편입/배지 전부 정상,
      0건일 때 섹션 숨김 확인) · 내부 링크 222개 · nav 18개 페이지 일치 ·
      태그 균형 · 미정의 CSS 0

### 글 올리는 법
1. `notes/<id>.html` 을 `_template.html` 복사해서 작성
2. `data/notes.json` 의 notes[] 에 한 덩어리 추가
   `{ id, title, category, date, status, summary }`
3. 새 카테고리가 필요하면 categories[] 에도 한 덩어리 추가

### 남은 것
- [ ] 첫 노트 올리기 — 지금은 notes[] 가 비어 있어 섹션이 안 보임
- [ ] notes/ 개별 글 sitemap 등재 (글 생기면)

---

## 철학을 ABOUT 소속으로 (2026-08-15)

두 자리의 역할을 갈랐다.
**ABOUT/철학** = 가르치는 일에 대해 이미 정리된 것.
**생각** = 아직 결론이 안 난, 지금 붙들고 있는 것 (노트 전용).

- [x] `about.html` 「오래 붙들고 있던 생각들」 섹션 신설 (참고하는 사람들 뒤)
  - [x] 여는 글을 블랙 블록으로 크게 세움 (프레이리 · 핀켈 · AI 침묵까지 요약)
  - [x] 01~06을 목차로 나열, 각 편 딥링크
  - [x] 「여기와 생각의 차이」 콜아웃 — 두 자리의 역할을 문장으로 못박음
- [x] `about.html` Next 카드에서 철학 제거(중복) → 「성장 프레임워크 넷」으로 교체
- [x] `philosophy.html` 앵커 7개 부여 — opening · p1~p6
- [x] `philosophy.html` eyebrow `About · Philosophy` 로, 리드를 ABOUT 연결로 다시 씀
- [x] `philosophy.html` 하단에 프레임워크 · ABOUT 복귀 경로 추가
- [x] 검증 — 내부 링크 228개 전수 OK(앵커 정합 포함) · 태그 균형 · 미정의 CSS 0
- [x] 배포 후 라이브 확인 (about 철학 섹션 · philosophy 앵커 7개)

---

## 남은 것 / 다음에

- [ ] topnav가 아직 7개 평면 나열 — 라우터와 같은 3분류(배우다/가르치다/만들다)로
      묶을지 검토. 전 페이지 헤더를 고쳐야 해서 별도 작업으로 분리함.
- [x] 라우터 15개 문장 중 어떤 게 실제로 눌리는지 이벤트 측정 붙이기
      (안 눌리는 문장 = 잘못 쓴 문장) → `router_symptom` 이벤트
- [ ] GA4 관리 화면에서 커스텀 측정기준 등록 — 등록 전에는 보고서에 안 뜬다
      이벤트 매개변수: track · symptom · slot · intent · tool · framework · kind ·
      note_title · tag · target_id · block · fw_axis
- [ ] 한 달 뒤 확인 — 도구 카드에서 read_first 대비 get_tool 비율,
      한 번도 안 눌린 증상 문장 추리기
- [ ] `og-image.png` — 신규 페이지용 개별 OG 이미지 (현재는 사이트 공통 1장 공유)
- [ ] `philosophy.html`이 topnav에 없음 — 여는 글이 생겼으니 노출 위치 재검토
      (현재는 index 본문 링크로만 접근)
- [ ] learning-design 3종은 px 기반 메트릭 유지 중 (사이트는 rem).
      색·타이포·크롬은 맞췄고, 간격 체계까지 통일할지는 미결

---

## 비공개 — 교육 사업 결론 노트 (2026-08-10 대화 정리)

> 공개 페이지에 올리지 않는 부분. 공개한 조각은 `notes/failure-curriculum.html`(주장·포맷·기록 리스트)와
> `solo-builders/recording-safety.html`(녹화 전 점검)로 분해했고, 아래는 사이트에 싣지 않는다.

### 타깃
- 기술력은 갖췄지만 "무엇을 만들어야 시장이 반응하는지" 모르는 iOS 개발자
- 1차 타깃 = 기존 수강생 8,000명+ (기본기를 마치고 다음 단계를 찾는 상태)

### 포지셔닝
- 성공 사례가 아니라 **실패의 해부**가 상품
- 완벽한 전문가가 아니라 **성실한 검증자**로서의 신뢰
- 만물 주머니를 라이브 케이스 스터디로 사용 (공개 범위는 미정)

### 다음 액션
- [ ] 가설 문서 1페이지 — 만물 주머니의 타깃과 존재 이유. 시리즈 0화가 된다
- [ ] 주간 회고 템플릿 확정 (망하지 않기 Gate 3 서식을 그대로 쓸지 결정)
- [ ] 녹화용 개발 환경 분리 → `recording-safety.html` 체크리스트 통과시키기
- [ ] 1화 녹화(4부 구조) 후 기존 수강생 채널에 먼저 공유해 초기 반응 관찰
- [ ] 4주 후 검증 — "이 포맷이 먹힌다"는 가설을 4주 데이터로 판정

### 결정 대기
- 수익·비용 공개 범위를 어디까지 열지
- 만물 주머니의 지표를 실명으로 공개할지, 익명 처리할지

---

## 구상 단계 — 배움의 프로세스 (5겹)

`learning-process.html`. **구상 중** 배지를 달아 두었고, 판매 문구·신청 경로는 붙이지 않았다.

미정인 것:
- [ ] 솔로 빌더스 멤버십과의 관계 — 개정판인가, 별개 라인인가
      (기존: 주간 체크인 + 목 21:00 위클리 + 무제한 Q&A + 코드리뷰 월 2회 /
       이 설계: 2주 오디오 + 1~2개월 워크숍 4시간 + 코칭 30분×2 + 간격 인출 + 연 2회 행사)
- [ ] 별개 라인으로 간다면 두 페이지가 서로 "이건 저것과 다르다"고 밝혀야 함
- [ ] 온라인 채널은 페이지에서 서비스명 없이 표기 중 (디스코드로 확정되면 그때 명시)
- [ ] 오디오 콘텐츠가 실제로 나오기 시작하면 구상 배지 제거
